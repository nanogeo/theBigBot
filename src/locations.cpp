...
void Locations::SetLightshadeLocations(Point3D start_location, BuildOrder build_order)
{
	...
	gateway_locations = { convert_location(Point2D(138.5, 66.5), swap),
							...
							convert_location(Point2D(113.5, 47.5), swap),
							convert_location(Point2D(113.5, 50.5), swap),
							convert_location(Point2D(113.5, 53.5), swap),
							convert_location(Point2D(115.5, 56.5), swap),
