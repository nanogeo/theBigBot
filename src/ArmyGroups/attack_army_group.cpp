...
namespace sc2 {
	...
	AttackArmyGroup::AttackArmyGroup(Mediator* mediator, PathManager attack_path, std::vector<UNIT_TYPEID> unit_types, 
		uint16_t desired_units, uint16_t max_units, uint16_t required_units, uint16_t min_reinforce_group_size) : ArmyGroup(mediator)
	{
	...
	bool AttackArmyGroup::MobilizeNewUnits(Units units)
	{
		...
		for (const auto& unit : units)
		{
			...
			mediator->SetUnitCommand(unit, A_MOVE, Utility::MedianCenter(all_units), 0);
	...
	void AttackArmyGroup::GroupUpNewUnits()
	{
		...
		mediator->SetUnitsCommand(new_units, A_MOVE, Utility::MedianCenter(new_units), 0);
	...
	AttackLineResult AttackArmyGroup::AttackLine()
	{
		...
		std::vector<Point2D> concave_positions = FindConcaveWithPrism(concave_target, concave_degree, prism_positions);
		...
		if (mediator->CheckUpgrade(U_BLINK))
		{
			for (const auto& incoming : incoming_damage)
			{
				if (incoming.first->unit_type == STALKER && incoming.second.unit_prio <= 2 && mediator->IsStalkerBlinkOffCooldown(incoming.first))
				{
					...
					mediator->SetUnitCommand(incoming.first, A_BLINK, standby_pos, 10);
		...
		for (const auto& assignment : unit_position_asignments)
		{
			if (mediator->GetAttackStatus(assignment.first) == false)
				mediator->SetUnitCommand(assignment.first, A_MOVE, assignment.second, 0);
		}
	...
	float AttackArmyGroup::CalculateDesiredRange(Units units, float unit_size)
	{
	...
	Point2D AttackArmyGroup::CalculateNewConcaveOrigin(Units close_enemies, Point2D concave_target, float desired_range, Point2D limit)
	{
		Point2D new_origin;
	...
	Point2D AttackArmyGroup::FindLimitToAdvance(std::vector<UNIT_TYPEID> unit_types, float extra_range, bool air, 
		uint16_t num_units_to_ignore)
	{
		...
		{
			if (std::find(unit_types.begin(), unit_types.end(), unit.first->unit_type) == unit_types.end())
				continue;
	...
	void AttackArmyGroup::FindNewConcaveOrigin()
	{
		...
		float desired_range = CalculateDesiredRange(basic_units, unit_size);
		...
		for (int i = 0; i < close_enemies.size(); i++)
		{
			if (Distance2D(concave_origin, close_enemies[i]->pos) > 15)
			{
				close_enemies.erase(close_enemies.begin() + i);
				i--;
			}
		...
		concave_origin = CalculateNewConcaveOrigin(close_enemies, concave_target, desired_range, limit);
	}

	std::vector<Point2D> AttackArmyGroup::FindConcaveWithPrism(Point2D concave_target, float concave_degree, std::vector<Point2D>& prism_positions)
	{
		...
		size_t num_units = basic_units.size();
		size_t num_prisms = warp_prisms.size();
	...
	void AttackArmyGroup::OraclesDefendArmy(Units basic_units)
	{
		if (oracles.size() == 0)
			return;
