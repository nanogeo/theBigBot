...
{
	mediator->SetUnitsCommand(all_units, A_MOVE, pylon_gap_pos, CommandPriority::low);
...
void DefendThirdZergArmyGroup::Run()
{
	for (const auto& unit : all_units)
	{
		...
		{
			mediator->SetUnitCommand(unit, A_STOP, CommandPriority::low);
		}
		else
		{
			if (Distance2D(unit->pos, pylon_gap_pos) > 1 && unit->orders.size() == 0)
			{
				mediator->SetUnitCommand(unit, A_MOVE, pylon_gap_pos, CommandPriority::low);
				mediator->SetUnitCommand(unit, A_HOLD_POSITION, CommandPriority::low, true);
			}
		}
	}
}
