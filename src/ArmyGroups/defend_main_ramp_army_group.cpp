		...
		if (walloff_gate && Distance2D(walloff_pos, walloff_gate->pos) < 1)
		{
			for (const auto& unit : all_units)
			{
				if (Distance2D(unit->pos, walloff_pos) > 3)
					mediator->SetUnitCommand(unit, A_MOVE, walloff_gate, CommandPriority::low);
				else
					mediator->SetUnitCommand(unit, A_ATTACK, walloff_gate, CommandPriority::low);
			}
		}
		else
		{
	...
	for (const auto& unit : all_units)
	{
		...
		if (unit->weapon_cooldown == 0)
		{
			...
			if (enemies_in_base.size() > 0)
			{
				...
				mediator->SetUnitCommand(unit, A_MOVE, closest->pos, CommandPriority::low);
			}
			else
			{
				mediator->SetUnitCommand(unit, A_MOVE, ramp_top, CommandPriority::low);
			}
		}
		else
		{
			...
			else
			{
				...
				if ((enemy_range < unit_range || unit->shield / unit->shield_max < .5) && Distance2D(closest_enemy->pos, unit->pos) <= enemy_range)
					mediator->SetUnitCommand(unit, A_MOVE, Utility::PointBetween(unit->pos, mediator->GetStartLocation(), unit_range), CommandPriority::normal);
		...
		Distance2D(Utility::NthClosestTo(enemy_units, forcefield_pos, 2)->pos, forcefield_pos) < 3)
	{
		...
		for (const auto& sentry : sentries)
		{
			if (sentry->energy >= 50)
			{
				...
				{
					mediator->SetUnitCommand(sentry, A_FORCEFIELD, forcefield_pos, CommandPriority::high);
		...
		{
			mediator->SetUnitCommand(Utility::ClosestTo(available_sentries, forcefield_pos), A_FORCEFIELD, forcefield_pos, CommandPriority::high);
		}
	}
}
